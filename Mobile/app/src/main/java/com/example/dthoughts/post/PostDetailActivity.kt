...
class PostDetailActivity : AppCompatActivity() {
    ...
    private fun showEditCommentDialog(comment: com.example.dthoughts.post.Comment) {
    ...
    private fun showDeleteConfirmation(comment: com.example.dthoughts.post.Comment) {
        ...
        lifecycleScope.launch {
            try {
                val response = RetrofitClient.apiService.updateComment(
                    commentId, 
                    com.example.dthoughts.post.CreateCommentRequest(user.id, content)
                )
    ...
    private fun sendLikeNotification(post: Post, currentUser: com.example.dthoughts.profile.User, isLiked: Boolean) {
    ...
    private fun sendCommentNotification(post: Post, currentUser: com.example.dthoughts.profile.User, commentText: String) {
        if (post.userId == currentUser.id) return // Don't notify yourself
